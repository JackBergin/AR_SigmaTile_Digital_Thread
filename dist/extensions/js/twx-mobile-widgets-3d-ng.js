...
(function () {
  ...
  let currentAreaTargetNavMode = false;
  let undoRedoTriggered = false;
  let lastLocationBeforeUndoRedo;
  ...
  const AllModelTargetTags = ['twx-dt-target-model', 'twx-dt-target-advanced-model', 'twx-dt-target-model-combined'];

  function isModelTargetTag(tagName) {
    return AllModelTargetTags.includes(tagName);
  }

  const All3DButtonTags = [
    'twx-dt-3dbutton',
    'twx-dt-3dpress-button',
    'twx-dt-3dimage-button',
    'twx-dt-3dcheckbox-button',
    'twx-dt-3dtoggle-button',
  ];

  function is3DButtonTag(tagName) {
    return All3DButtonTags.includes(tagName);
  }

  const All3DPanelTags = ['twx-dt-3dvideo', 'twx-dt-3daudio', 'twx-dt-3dpanel', 'twx-dt-3ddocument'];

  function is3DPanelTag(tagName) {
    return All3DPanelTags.includes(tagName);
  }
  ...
  function renderer($rootScope, $timeout, thingViewPath) {
    ...
    function cvApiReady() {
      ...
      if (!session) {
        ...
        if (els && els.length) {
          ...
          view.SetKeyboardNavigation(true);
    ...
    function updateRuntimeModelTargetLocation(model, modelTargetWidget) {
      ...
      if (box.valid) {
        ...
        if (typeof modelTargetWidget.AdjustVectorImagesToModel === 'function') {
          modelTargetWidget.AdjustVectorImagesToModel(box, modelLocation);
        }
      }
    ...
    function getDerivedHidden(obj) {
      let widget = obj.GetWidget();
      if (widget.parentModelId) {
        //model items don't need the other logic since thier hierarchy is controlled by Thingview
        return obj.properties.hidden;
      }
      if (isInherited(obj.properties.hidden)) {
        if (widget && widget.parent) {
          let parent = vrSession[widget.parent];
          if (parent) {
            return getDerivedHidden(parent);
          }
        }
        return false;
      } else {
      ...
      if (widget) {
        if (widget.loaded) {
          ...
          if (obj.properties !== undefined) {
            ...
            let hidden = getDerivedHidden(obj);
            if (hidden !== undefined) {
              ...
              } else if (widget.name.includes('-occluder')) {
                // Area target NavMesh should not be visible in preview
                widget
                  .GetRootShapeInstance()
                  .SetVisibility1(false, Module.ChildBehaviour.INCLUDE, Module.InheritBehaviour.OVERRIDE, 0);
              } else {
                widget.SetVisibility(!hiddenBool);
                const inheritedClassCaster = Module.InheritedClassCaster.Create();
                let model = undefined;
                if (obj.GetType() !== 'Image') {
                  model = inheritedClassCaster.CastModelBaseToModel(widget);
                }
                // if model is undefined then widget is an Image.
                if (model) {
                  const shapeInstances = model.GetShapeInstances(true);
                  // if we try to hide the model - for each child node with visibility set to 1
                  // we remove the visibility property, then it gets property from parent and not overriding it
                  for (let i = 0; i < shapeInstances.size(); i++) {
                    let shapeInstance = shapeInstances.get(i);
                    let visibility = shapeInstance.GetVisibility1();
                    if (visibility.valid && visibility.val && hiddenBool) {
                      shapeInstance.UnsetVisibility1(Module.ChildBehaviour.IGNORED, -1);
                    }
                    shapeInstance.delete();
                  }
                }
                inheritedClassCaster.delete();
    ...
    function applyShader(obj, shader) {
      ...
      if (widget) {
        ...
        if (parsedShader) {
          ...
          console.log('shader definition not found');
        }
      }
    }
      ...
      widget.LoadModel = function (url, successCb, errorCb) {
        ...
        app.LoadModelFromURLWithCallback(widget, url, true, true, false, function (success, isStructure, errors) {
          ...
          if (isStructure) {
            ...
            const loadSource = widget.GetStructureLoadSource();

            if (
              loadSource &&
              loadSource.GetPropertiesAsJSON &&
              typeof loadSource.GetPropertiesAsJSON === 'function' &&
              window &&
              window.PTC &&
              window.PTC.Metadata &&
              typeof window.PTC.Metadata.fromData === 'function'
            ) {
              // STUD-1996: dynamic metadata code
              // const metadata = loadSource.GetPropertiesAsJSON();
              // window.PTC.Metadata.fromData(widget.name, JSON.parse(metadata)).then((metadata) => {
              //   const metadataChangedEvent = new CustomEvent(`${metadata.id}_metadataChanged`, { detail: metadata });
              //   document.dispatchEvent(metadataChangedEvent);
              // });
            }
     ...
     * @param {function} errorCb error callback
     */
    this.add3DVideo = function (params, successCb, errorCb) {
      this.add3DImage(
        {
    ...
    this.setVideo = function (params, successCb, errorCb) {
      // on Preview we put a placeholder image
      if (params.src.startsWith('data:image')) {
        this.setTexture(params.id, params.src);
      }
    };

    this.add3DDocument = function (params, successCb, errorCb) {
      const internalSuccessCb = function () {
        ...
        ctx.setDocument(newParams);
        if (typeof successCb === 'function') {
          successCb();
        }
      };

      const internalErrorCb = function () {
        errorCb('Could not create a page image for the document');
      };
    ...
    this.setDocument = function (params, successCb, errorCb) {
      // This base64 string represents a black transparent pixel RGBA(0, 0, 0, 0).
      const blankPixel =
        'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=';

      let src = params.src;

      const internalSuccessCb = function (msg) {
        if (typeof successCb === 'function') {
          successCb();
        }
        $rootScope.$broadcast('documentloadednative', params.id, '', src);
        console.log(msg);
      };

      const internalErrorCb = function (msg) {
        if (typeof errorCb === 'function') {
          errorCb();
        }
        $rootScope.$broadcast('documentfailednative', params.id, '', src);
        console.log(msg);
      };

      if (!widget3dUtils.isSupportedDocument(src)) {
        // Set to a transparent pixel.
        ctx.setTexture(params.id, blankPixel);
        console.log('Rendering a blank page');
        if (!src.endsWith('.pdf') && !(src === '' || src === undefined)) {
          internalErrorCb('The file ' + params.src + ' is of an unsupported type.');
        }
        return;
      }
      let obj = this.GetObject(params.id);
      if (!obj) {
        internalErrorCb('Error setting document, ' + params.id + ' does not exist.');
        return;
      }
      if (obj.currentSource === src) {
        console.log('Cannot update the document src to the same src.');
        return;
      }
      obj.documentHandlerPromise = undefined;
      obj.documentPropertiesObject = undefined;

      let documentHandler = undefined;
      try {
        documentHandler = pdfjsLib.getDocument(src);
      } catch (err) {
        internalErrorCb('Failed to create a file handler for "' + src + '". (renderer)');
        return;
      }
      obj.documentHandlerPromise = documentHandler.promise
        .then(async function (docProps) {
          obj.documentPropertiesObject = docProps;
          let newParams = {
            id: params.id,
            pageNumber: 1,
          };
          internalSuccessCb(src + ' loaded succesfully. (renderer)');
          ctx.navigateToPage(newParams);
        })
        .catch((_) => {
          obj.documentHandlerPromise = undefined;
          obj.documentPropertiesObject = undefined;
          // Set to a transparent pixel.
          ctx.setTexture(params.id, blankPixel);
          internalErrorCb('Error obtaining properties of "' + src + '". Make sure the file exists. (renderer)');
        });
    };

    this.navigateToPage = async function (params, successCb, errorCb) {
      let obj = this.GetObject(params.id);
      if (!obj) {
        console.error('Error rendering page, ' + params.id + ' does not exist.');
        if (typeof errorCb === 'function') {
          errorCb();
        }
        return;
      }
      if (!obj.documentPropertiesObject) {
        console.error('Error rendering page, ' + params.id + ' does not contain a document.');
        if (typeof errorCb === 'function') {
          errorCb();
        }
        return;
      }
      // Do not allow two render tasks to run simultaneously.
      if (obj.currentPagePromise) {
        await obj.currentPagePromise;
      }
      obj.currentPagePromise = obj.documentPropertiesObject
        .getPage(params.pageNumber)
        .then(async function (page) {
          ...
          await renderTask.promise
            .then(function () {
              ctx.setTexture(params.id, canvas.toDataURL('image/png'));
              if (typeof successCb === 'function') {
                successCb();
              }
              console.log('Rendered page ' + params.pageNumber + ' for ' + params.id + '.');
            })
            .catch((_) => {
              if (typeof errorCb === 'function') {
                errorCb();
    ...
    this.add3DAudio = function (params, successCb, errorCb) {
      // just placeholder, do nothing
    };

    this.setAudio = function (params, successCb, errorCb) {
      // just placeholder, do nothing
    };
    ...
    this.add3DButton = function (buttonParams, successCb, errorCb) {
      if (session) {
        ...
        var width = buttonParams.width; // < widget3dUtils.ButtonMinSize ? widget3dUtils.ButtonMinSize : buttonParams.width;
        var height = buttonParams.height; // < widget3dUtils.ButtonMinSize ? widget3dUtils.ButtonMinSize : buttonParams.height;
      ...
      if (session) {
        ...
        if (src.startsWith('vuforia-vumark://')) {
          if (
            ...
            if (this._experienceDefinedTargets[id].guide.endsWith('app/resources/Default/thing_code_phantom.png')) {
              // Preview should show placeholder_thingmark
              guideURL = '/extensions/images/placeholder_thingmark.png';
              markerType = 'ThingMark';
            } else {
              guideURL = this._experienceDefinedTargets[id].guide;
              markerType = 'VuMark Target';
            }
          } else {
            // if the above condition fails for VuMark; use thingmark placeholder.
            guideURL = '/extensions/images/placeholder_thingmark.png';
            markerType = 'ThingMark';
          }
          ...
          if (src.includes('modelTargetType=')) {
            const modelTargetType = src.split('modelTargetType=')[1];
            guideURL =
              modelTargetType === 'standard'
                ? '/extensions/images/AMT_standard.svg'
                : '/extensions/images/AMT_advanced.svg';
          }
          markerType = 'Model Target';
        } else if (src.startsWith('vuforia-area://')) {
          guideURL = '/extensions/images/area-target.png';
          markerType = 'Area Target';
        } else if (
          this._experienceDefinedTargets &&
          this._experienceDefinedTargets[id] &&
          this._experienceDefinedTargets[id].guide
    ...
    this.addTargetRepresentation = function (params) {
      if (params.src) {
        const targetPresentationId = params.target + '_representation';
        const successCallback = (info) => {
          const widget = vrSession[targetPresentationId].GetWidget();
          if (widget) {
            // This is temporary fix to turn of lighting for area target under DT-32996.
            // This should be removed when TV has support for non-cad shaders for area target - DT-32783
            widget.SetTMLShaderInt('pvg_reflectanceModel', 0);
            updateTargetRepresentationLocation(widget);
          }
        };
        this.addPVS(params.tracker, targetPresentationId, params.src, undefined, undefined, successCallback);
      }
    };

    function updateTargetRepresentationLocation(widget) {
      // This code is only for area targets - they only have representations.
      const box = widget.CalculateBoundingBox(new Module.VectorString());
      if (box.valid) {
        const props = {
          x: 0,
          y: -box.min.y,
    ...
    this.setRotation = function (name, rx, ry, rz) {
      var sessionObj = this.GetObject(name);
      if (sessionObj !== undefined) {
        if (sessionObj.GetType() === 'Area Target') {
          rx = Number(rx) - 90; // for rotating the image mark without rotating the target
        }
    ...
    this.setDimensions = function (name, params) {
      const width = Number(params.width);
      const height = Number(params.height);
      var sessionObj = this.GetObject(name);
      if (sessionObj !== undefined && !isNaN(width) && !isNaN(height)) {
    ...
    function renderMockScreenshotForPreview(width, height) {
      ...
      // to test for changes.
      mockImageCounter += 1;
      const text = window.parent.i18next.t('@ptc/ves-ar-extension:preview-2d-eyewear-mock-image-text', {
  ...
  function twxDt3dView($rootScope, $timeout, $window, widget3dUtils) {
    ...
    var linker = function (scope, element, attrs) {
      var ctrl = scope.ctrl;
      ctrl.countTarget = 0;
      ctrl.handleErrorModelTarget = false;
      ...
      function isHorizontalSurface(pickedLocation) {
        return pickedLocation.orientation.y > 0.85;
      }

      function navigateOnAreaTarget(pickedLocation) {
        if (isHorizontalSurface(pickedLocation)) {
          let finalLocation = ctrl.myView.GetViewLocation();
          finalLocation.position = pickedLocation.position;
          // move up vertically the camera 1.75 meter
          finalLocation.position.y += 1.75;
          ctrl.myView.SetViewLocation(finalLocation);
        }
      }

      function navigate(x, y) {
        ctrl.myView.DoPickWithCallback(x, y, true, true, function (pickResult) {
          if (pickResult.IsValid()) {
            const pickedLocation = pickResult.GetLocation();
            const widget = ctrl.GetWidgetFromPickResult(pickResult);
            if (widget && widget.getWidgetTagName() === 'twx-dt-target-area') {
              navigateOnAreaTarget(pickedLocation);
            }
          }
        });
      }

      function selectAreaTarget(x, y) {
        const isAreaTargetSelected = ctrl.myView.DoPickWithCallback(x, y, true, true, function (pickResult) {
          if (pickResult.IsValid()) {
            const widget = ctrl.GetWidgetFromPickResult(pickResult);
            if (widget && !ctrl.selectedAreaTarget && widget.getWidgetTagName() === 'twx-dt-target-area') {
              $rootScope.$broadcast('select3DObj', { ctrlWidget: widget.$cvWidget.ctrlWidget, partId: '' });
              ctrl.selectedAreaTarget = widget.$cvWidget.ctrlWidget;
              return true;
            }
          }
          return false;
        });
        if (!isAreaTargetSelected) {
          if (ctrl.selectedAreaTarget) {
            $rootScope.$broadcast('deselect3DObj', { ctrlWidget: ctrl.selectedAreaTarget, partId: '' });
            ctrl.selectedAreaTarget = null;
          }
        }
      }

      function dblClickListener(event) {
        navigate(event.originalEvent.clientX, event.originalEvent.clientY);
        event.stopPropagation();
      }

      function clickListener(event) {
        // Clicks inside of toolbar also get captured here - ignore them, we're only interested in clicks on the canvas.
        if (event.target.closest('.orbit-control-toolbar')) {
          return;
        }
        ...
        selectAreaTarget(event.originalEvent.clientX, event.originalEvent.clientY);
      ...
      element.off('dblclick', dblClickListener);
      element.on('click', clickListener);
      element.on('dblclick', dblClickListener);
      ...
      if ($window.twx) {
        ...
        $window.twx.get3DWidgetIdAtPoint = function (x, y) {
          var selectPromise = new Promise(function (resolve, reject) {
            ctrl.myView.DoPickWithCallback(x, y, true /* invert */, true /* include markups */, function (pickResult) {
              ...
              if (
                widget &&
                widget.getWidgetTagName() !== 'twx-dt-target-model' &&
                widget.getWidgetTagName() !== 'twx-dt-target-advanced-model' &&
                widget.getWidgetTagName() !== 'twx-dt-target-model-combined'
              ) {
        ...
        $window.twx.onCanvasUndoRedoBegin = function () {
          currentAreaTargetNavMode = ctrl.areaTargetNavMode;
          undoRedoTriggered = true;
          if (typeof ctrl.myView !== 'undefined' && ctrl.myView.GetViewLocation) {
            lastLocationBeforeUndoRedo = ctrl.myView.GetViewLocation();
          }
        };

        $window.twx.onCanvasUndoRedoEnd = function () {
          const isAreaTargetExists = document.querySelector('twx-dt-target-area');
          if (lastLocationBeforeUndoRedo && isAreaTargetExists) {
            if (currentAreaTargetNavMode) {
              ctrl.updateNavMode(currentAreaTargetNavMode);
            }
            setTimeout(() => ctrl.myView.SetViewLocation(lastLocationBeforeUndoRedo), 250);
            setTimeout(() => (undoRedoTriggered = false), 1000);
            $rootScope.$apply();
          }
        };
      }
      ...
      controller: function ($scope, $element, $http) {
        ...
        ctrl.getWidgetFactory = function (widgetTag) {
          var stdFactories = {
            ...
            'twx-dt-target-model-combined': ctrl.addModelTargetCombined,
            'twx-dt-target-area': ctrl.addAreaTarget,
            'twx-dt-image': ctrl.addImageMarker,
            'twx-dt-waypoint': ctrl.addWaypoint,
            ...
            'twx-dt-3ddocument': ctrl.add3DDocument,
          ...
          if (ctrlWidget && ctrl.myWidgets[ctrlWidget.widgetId]) {
            ...
            if (ctrlWidget.$cvWidget && ctrlWidget.$cvWidget.isCustomWidget) {
              ctrlWidget.$cvWidget.children.forEach(function (child) {
                ...
                if (tagName === 'twx-dt-image' || tagName === 'twx-dt-label' || tagName === 'twx-dt-waypoint') {
            ...
            if (tagName === 'twx-dt-model') {
              ...
              // Area target nav mode can be used only when area target is on canvas
              // when removing area target, ensure that we exit nav mode
              if (tagName === 'twx-dt-target-area') {
                ctrl.updateNavMode(false);
                // when target is being removed update count
                ctrl.countTarget = ctrl.countTarget - 1;
              }
            } else if (
              tagName === 'twx-dt-image' ||
              tagName === 'twx-dt-label' ||
              tagName === 'twx-dt-waypoint' ||
              ...
              if (ctrl.countTarget > 0 && ctrl.handleErrorModelTarget === false) {
                // when target is being removed update count
                ctrl.countTarget = ctrl.countTarget - 1;
              }

              if (ctrl.myScene && ctrlWidget.$cvWidget) {
                // Delete the Vector images for the combined model target.
                if (typeof ctrlWidget.$cvWidget.DeleteVectorImages === 'function') {
                  ctrlWidget.$cvWidget.DeleteVectorImages();
                }
                ctrl.myScene.DeleteImageMarker(ctrlWidget.$cvWidget);
              }

              if (ctrl.myScene && ctrlWidget.$waypointLabel) {
                ctrl.myScene.DeleteImageMarker(ctrlWidget.$waypointLabel);
                ctrlWidget.$waypointLabel.delete();
                ctrlWidget.$waypointLabel = null;
              }
            ...
            // Area target is big compared to other widgets, when it's removed and floor is resized,
            // whole scene will become typically very small. To fix it do a zoom all to adjust zoom levels.
            if (tagName === 'twx-dt-target-area') {
              doZoomAllWhenResizingFloor = true;
            }
        ...
        let doZoomAllWhenResizingFloor = false;
        var resizeFloorDebounce = _.debounce(function () {
          if (ctrl.myScene) {
            ...
            if (doZoomAllWhenResizingFloor) {
              doZoomAllWhenResizingFloor = false;
              ctrl.doZoomAll();
            }
          }
        }, 100);
        ...
        ctrl.detachChildModelItems = function (ctrlWidget) {
          // Need to clone array for iteration as removeObject called below is splicing it.
          ctrlWidget.$cvWidget.modelItems.slice().forEach((modelItem) => {
        ...
        ctrl.updateNavMode = (areaTargetSelected, resetCameraYLocation = false) => {
          // resetCameraYLocation - when toggle from EXPLORE mode to (VUFORIA) default mode, reset camera (Y axis) to initial location?
          //If area target is selected turn explore mode on
          if (areaTargetSelected) {
            ctrl.areaTargetNavMode = true;
            if (typeof ctrl.myView !== 'undefined') {
              ctrl.myView.SetNavigationMode1(Module.NavMode.EXPLORE, true);
            }
          } else {
            ctrl.areaTargetNavMode = false;
            if (typeof ctrl.myView !== 'undefined') {
              ctrl.myView.SetNavigationMode1(Module.NavMode.VUFORIA, resetCameraYLocation);
            }
          }
          ctrl.myView.SetKeyboardNavigation(ctrl.areaTargetNavMode);
        };
        ...
        function updateWidgetTransform(ctrlWidget, args) {
          ...
          if (
            (ctrl.getParentActiveElement().tagName === 'IFRAME' || args.forcePropsUpdate) &&
            (!ctrlWidget.$widgetDef || ctrlWidget.$widgetDef.elementTag !== 'twx-dt-target-model-combined')
          ) {
            ctrlWidget.setProps({
        ...
        unRegLoaded = $scope.$on('loaded3DObj', function (event, args) {
          ...
          if (ctrlWidget.$cvWidget) {
            ...
            if (tagName === 'twx-dt-model') {
              ...
              ctrlWidget.$cvWidget.SetAssetsAppearance();
              ctrlWidget.$cvWidget.UpdateAssetsPosition(
                {
              ...
              ctrlWidget.$cvWidget.SetPickable(true);
              ctrlWidget.$cvWidget.Select(false);
              // This is temporary fix to turn of lighting for area target under DT-32996.
              // This should be removed when TV has support for non-cad shaders for area target - DT-32783
              ctrlWidget.$cvWidget.SetTMLShaderInt('pvg_reflectanceModel', 0);
            }
            ...
            if (tagName === 'twx-dt-target' || tagName === 'twx-dt-target-image' || tagName === 'twx-dt-waypoint') {
        ...
        ctrl.updateObject = function (ctrlWidget, props, changedProps, initializeWidget) {
          ...
          if (ctrlWidget && cvWidget) {
            ...
            if ('modelTargetType' in changedProps) {
              if (typeof cvWidget.SetGizmos === 'function') {
                cvWidget.SetGizmos();
              }
            }
            ...
            if ('src' in changedProps) {
              ...
              } else if (tagName === 'twx-dt-3ddocument') {
                ctrlWidget.$cvWidget.UpdateSource(props.src);
              }
            }
            if ('model' in changedProps) {
              if (isModelTargetTag(tagName)) {
                ...
                if (modelWidget) {
                  // Ignore Vector images for twx-dt-target-model-combined.
            ...
            if ('widgetName' in changedProps && ctrlWidget.$waypointLabel && ctrlWidget.labelProps) {
              ctrlWidget.labelProps.text = changedProps.widgetName;
              GenerateMarkupImage(ctrlWidget.$waypointLabel, ctrlWidget.labelProps);
            }
        ...
        ctrl.beforeDestroy = function () {
          if (ctrl.mySession) {
            ...
            if (cvApi.DestroyApplication) {
              cvApi.DestroyApplication(ctrl.myApp);
            }
            ctrl.mySession.delete();
            ctrl.mySession = null;
            ctrl.myApp.delete();
            ctrl.myApp = null;
            ctrl.myScene.delete();
        ...
        ctrl.cvApiReady = function () {
          ...
          ctrl.myApp.SetReviseSelectionCallback(function reviseSelectionCallback(selectable, scene, selListId) {
            const si = selectable.GetSelectableAsShapeInstance();
            if (si) {
              const siModel = si.GetModel();
              if (!siModel) {
                si.delete();
                return;
              }

              const widget = ctrl.myWidgets[siModel.GetUserId()];
              if (!widget) {
                console.error('Missing widget', siModel.GetUserId());
                siModel.delete();
                si.delete();
                return;
              }

              const tagName = widget.getWidgetTagName();
              if (tagName === 'twx-dt-target-area') {
                const atCvWidget = widget.$cvWidget;
                const sisel = siModel.GetShapeInstance();
                const simodel = atCvWidget.GetShapeInstance();
                if (sisel && simodel && sisel.GetUniqueId() === simodel.GetUniqueId()) {
                  // For area target pickable must be set to true in order to allow double click on it.
                  // Therefore, The purpose is to remove the selection from an area
                  // target when it is selected
                  selectable.Reset();
                }
              } else {
                // Component Hide mode is only applicable to models.
                if (ctrl.getCompHideMode() && tagName === 'twx-dt-model') {
                  // Do nothing here, TV will by default select parts.
                } else {
                  // Use authoring mode for all other widgets besides models.
                  // Need to set model as the selectable unless we have a model item defined for the part that is being selected.
                  // If we don't do that we would be able to select parts that do not have model items created for them.
                  if (
                    !(widget.$cvWidget.modelItems || []).some(
                      // Shape instance might not be available after model's resource property is changed - model items
                      // will remain even than.
                      (modelItem) =>
                        modelItem.GetShapeInstance() && modelItem.GetShapeInstance().GetUniqueId() === si.GetUniqueId()
                    )
                  ) {
                    selectable.SetModelAsSelectable(siModel);
                  }
                }
              }
              siModel.delete();
          ...
          ctrl.myView.SetCameraMoveCallback((cameraLocation) => {
            // allow interested widgets to subscribe to this camera movement event
            // without having to override this listener
            $rootScope.$broadcast('moveCamera', cameraLocation);
          });
        ...
        ctrl.updateWidgetsOnStart = function updateWidgetsOnStart() {
          ...
          angular.forEach(_.sortBy(ctrl.myWidgets, widgetTypeModelFirstSorter), function (ctrlWidget, widgetId) {
            ...
            //updateObject causes double loading issues with AreaTarget
            if (!isAreaTargetTag(ctrlWidget)) {
        ...
        ctrl.toggleNavMode = function () {
          if (ctrl.hasAreaTarget) {
            ctrl.updateNavMode(!ctrl.areaTargetNavMode);
          }
        };
        ...
        ctrl.doZoomAll = function () {
          if (ctrl.myView && !undoRedoTriggered) {
        ...
        // Zoom To Parts
        ctrl.doZoomParts = function (modelWidget) {
          // Since GetInstance1() is depracated, on TV 59 switch to modelWidget.GetRootShapeInstance().ZoomToShapeInstances()
          if (!undoRedoTriggered) {
            let idPathVector = new Module.ShapeInstanceVector();
            idPathVector.push_back(modelWidget.GetRootShapeInstance());
            ctrl.myView.ZoomToShapeInstances(idPathVector, 0);
            idPathVector.delete();
          }
        };
        ...
        function addModelFn(ctrlWidget, dropCoords, params) {
          ...
          widget.modelItems = [];
          if (params && params.placeAtOrigin) {
            widget.placeAtOrigin = true;
          }
          ...
          widget.LoadModel = function (src) {
            ...
            if (!src || src === '' || src.slice(-1) === '/') {
              if (!widget.isPlaceHolder) {
                ...
                let modelSrc;
                if (isAreaTargetTag(widget.ctrlWidget)) {
                  modelSrc = 'extensions/images/placeholder_area_target.glb';
                } else {
                  modelSrc = 'extensions/images/vse-3d-model.ol';
                }
          ...
          widget.ModelLoadedHandler = function (success, isStructure, errors) {
            if (success) {
              ...
              // Area Target is bigger so zoom should be adjusted to model
              if (isAreaTargetTag(widget.ctrlWidget)) {
                ctrl.doZoomParts(widget);
              }

              if (isStructure) {
                // Add model items to this model in case they haven't been added already.
                _.forEach(ctrl.myWidgets, (widgetItem) => {
                  if (widgetItem.getWidgetTagName() === 'twx-dt-modelitem') {
                    let model = widgetItem.getProp('model');
                    if (
                      widget.ctrlWidget.widgetId === model &&
                      !_.find(widget.modelItems, (item) => item.ctrlWidget.widgetId === widgetItem.widgetId)
                    ) {
                      widget.modelItems.push(widgetItem.$cvWidget);
                    }
                  } else if (widgetItem.getWidgetTagName() === 'twx-dt-waypoint') {
                    let model = widgetItem.getProp('model');
                    if (widget.ctrlWidget.widgetId === model && widgetItem.delegate.modelCtrlWidget === undefined) {
                      console.log('widgetItem', widgetItem, 'modelCtrlWidget', widgetItem.delegate.modelCtrlWidget);
                      widgetItem.delegate.modelCtrlWidget = widget.ctrlWidget;
                      widgetItem.delegate.waypointPlacementCompleted(widgetItem, widgetItem.delegate.modelCtrlWidget);
                    }
                  }
                });
          ...
          widget.CalculateOffset = function () {
            if (!this.originOffset) {
              ...
              if (box.valid) {
                ...
                var location = widget.GetLocation();
                widget.originOffset = {
                  x: widget.placeAtOrigin ? 0 : (box.min.x + box.max.x) / 2 - location.position.x,
                  y: widget.placeAtOrigin ? box.min.y : box.min.y - location.position.y,
                  z: widget.placeAtOrigin ? 0 : (box.min.z + box.max.z) / 2 - location.position.z,
                };
          ...
          widget.PlacementPickHandler = function (pickResult) {
            ModelPlacementPickHandler(widget, pickResult, ctrl);
          };
              ...
              if (is3DButtonTag(tagName) || is3DPanelTag(tagName)) {
                ...
                if (model) {
                  ...
                  if (model.isPlaceHolder) {
                    ...
                    window.alert(
                      window.i18next.t('@ptc/ves-ar-extension:Drop On Canvas Failed', {
                        widget: window.i18next.t('@ptc/ves-ar-extension:Model Item'),
                      })
                    );
        ...
        ctrl.addAreaTarget = (ctrlWidget, dropCoords) => {
          ctrl.countTarget = ctrl.countTarget + 1;
          // Ignoring dropCoords because area target always gets placed at the origin.
          return ctrl.addModel(ctrlWidget, { x: 0, y: 0 }, { placeAtOrigin: true });
        };
        ...
        ctrl.addModelTarget = function (ctrlWidget, dropCoords) {
          ...
          widget.PlacementPickHandler = function (result) {
            ...
            if (model) {
              if (ctrl.countTarget >= 1) {
                TargetUtils.canTargetBeAdded(ctrl, undefined, widget, true);
              }
              ...
              if (tagName !== 'twx-dt-model') {
                ctrl.handleErrorModelTarget = true;
          ...
          widget.HandleModelTargetError = function () {
            widget.ctrlWidget.showAlertPopup(
              window.location.host,
              window.i18next.t('body-warning-message-without-position')
            );
            $rootScope.$broadcast('loadError3DObj', { ctrlWidget: widget.ctrlWidget });
            widget.ctrlWidget.remove();
            delete widget.ctrlWidget;
        ...
        // Add a Model Target Combined to the Canvas
        ctrl.addModelTargetCombined = function (ctrlWidget, dropCoords) {
          const VECTOR_DISTANCE_COEFFICIENT = 0.75;
          const UNSELECTED_DIAMOND_IMAGE = 'extensions/images/Model_Target_Combined_Diamond_Unselected.png';
          const SELECTED_DIAMOND_IMAGE = 'extensions/images/Model_Target_Combined_Diamond_Selected.png';
          const SQUARE_BASEPLATE = 'extensions/images/Model_Target_Combined_Square_Base.png';
          const CIRCLE_BASEPLATE = 'extensions/images/Model_Target_Combined_Circle_Base.png';
          const STANDARD = 'standard';
          const ADVANCED = 'advanced';

          let currentDeltaDistance = undefined;
          let yDelta = undefined;

          var widget = CreateImageMarker(ctrlWidget, dropCoords, ctrl.myScene);
          let ImageMarkerObserverClass = Module.ImageMarkerObserver.extend('ImageMarkerObserver', {
            // Location Callback
            OnLocationChange: function (loc) {
              if (!ctrlWidget.vectorImages) {
                return;
              }
              if (!currentDeltaDistance) {
                return;
              }

              widget.RepositionVectorImages(loc);
            },
            OnSelection: function (selected, selectType, type, instance, idPath) {
              if (selectType === Module.SelectionList.PRIMARYSELECTION) {
                widget.SelectImages(selected);
              }
            },
          });
          ...
          widget.SelectImages = function (selected) {
            const gizmoImage = selected ? SELECTED_DIAMOND_IMAGE : UNSELECTED_DIAMOND_IMAGE;

            for (const imgKey in ctrlWidget.vectorImages) {
              let currentVectorImage = ctrlWidget.vectorImages[imgKey];
              const imageHeight = currentVectorImage.GetSize().height;
              currentVectorImage.LoadFromURLWithCallback(gizmoImage, () => {
                currentVectorImage.SetHeight(imageHeight);
              });
            }
          };

          widget.ShouldRegenerateGizmos = function () {
            const modelTargetType = ctrlWidget.me.modelTargetType;
            const standardGizmoVis = ctrlWidget.vectorImages.standardVectorImage.GetVisibility();

            return (
              (modelTargetType === STANDARD && !standardGizmoVis) || (modelTargetType === ADVANCED && standardGizmoVis)
            );
          };

          widget.CreateVectorImages = function () {
            if (ctrlWidget.vectorImages) {
              if (widget.ShouldRegenerateGizmos()) {
                widget.SetGizmos();
              }
              return;
            }

            ctrlWidget.vectorImages = {
              standardVectorImage: ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords),
              topVectorImage: ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords),
              bottmVectorImage: ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords),
              leftVectorImage: ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords),
              rightVectorImage: ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords),
            };

            for (const imgKey in ctrlWidget.vectorImages) {
              let currentVectorImage = ctrlWidget.vectorImages[imgKey];
              // Fixes a glitch that makes the image pickable.
              GenerateMarkupImage(currentVectorImage, {
                src: '',
              });
              currentVectorImage.SetPickable(false);
              GenerateMarkupImage(currentVectorImage, {
                src: UNSELECTED_DIAMOND_IMAGE,
              });
              currentVectorImage.SetDrawType(Module.DrawType.WORLD_SPACE_BILLBOARD);
            }
          };

          widget.AdjustVectorImagesToModel = function (bbox, loc) {
            if (!ctrlWidget.vectorImages) {
              widget.CreateVectorImages();
            }

            const maxWidth = Math.max(bbox.max.x - bbox.min.x, bbox.max.z - bbox.min.z);
            currentDeltaDistance = VECTOR_DISTANCE_COEFFICIENT * maxWidth;

            yDelta = bbox.max.y - bbox.min.y;
            const vectorHeight = yDelta !== 0 && yDelta > maxWidth ? yDelta / 5 : maxWidth / 5;

            for (let imgKey in ctrlWidget.vectorImages) {
              let currentVectorImage = ctrlWidget.vectorImages[imgKey];
              currentVectorImage.SetHeight(vectorHeight);
            }

            widget.RepositionVectorImages(loc);
          };

          widget.RepositionVectorImages = function (loc) {
            if (!ctrlWidget.vectorImages) {
              widget.CreateVectorImages();
            }

            const xCenter = loc.position.x;
            let yCenter = loc.position.y;
            let zCenter = loc.position.z;

            let topPos = zCenter + currentDeltaDistance;
            let bottomPos = zCenter - currentDeltaDistance;
            let leftPos = xCenter - currentDeltaDistance;
            let rightPos = xCenter + currentDeltaDistance;

            ctrlWidget.vectorImages.standardVectorImage.SetPosition(rightPos, yCenter, topPos);
            ctrlWidget.vectorImages.topVectorImage.SetPosition(xCenter, yCenter, topPos);
            ctrlWidget.vectorImages.bottmVectorImage.SetPosition(xCenter, yCenter, bottomPos);
            ctrlWidget.vectorImages.leftVectorImage.SetPosition(leftPos, yCenter, zCenter);
            ctrlWidget.vectorImages.rightVectorImage.SetPosition(rightPos, yCenter, zCenter);
          };

          widget.SetGizmos = function () {
            if (!ctrlWidget.vectorImages) {
              return;
            }
            const isStandard = ctrlWidget.me.modelTargetType === STANDARD;
            const baseImage = isStandard ? SQUARE_BASEPLATE : CIRCLE_BASEPLATE;

            const baseHeight = widget.GetSize().height;
            widget.LoadFromURLWithCallback(baseImage, () => {
              widget.SetHeight(baseHeight);
            });

            ctrlWidget.vectorImages.standardVectorImage.SetVisibility(isStandard);
            ctrlWidget.vectorImages.topVectorImage.SetVisibility(!isStandard);
            ctrlWidget.vectorImages.bottmVectorImage.SetVisibility(!isStandard);
            ctrlWidget.vectorImages.leftVectorImage.SetVisibility(!isStandard);
            ctrlWidget.vectorImages.rightVectorImage.SetVisibility(!isStandard);
          };

          widget.DeleteVectorImages = function () {
            for (const imgKey in ctrlWidget.vectorImages) {
              ctrl.myScene.DeleteImageMarker(ctrlWidget.vectorImages[imgKey]);
              ctrlWidget.vectorImages[imgKey].delete();
            }
          };
          ...
          widget.PlacementPickHandler = function (result) {
            ...
            if (model && model.ctrlWidget && model.ctrlWidget.getWidgetTagName() === 'twx-dt-model') {
              if (sessionStorage.getItem('checkTarget' + currentView) === 'true') {
                sessionStorage.setItem('elementTag', 'twx-dt-target-model-combined');
                let modelTmp = model;
                TargetUtils.canTargetBeAdded(ctrl, undefined, widget, true, true).then((widget) => {
                  modelTmp.ctrlWidget.modelTarget = widget;
                });
              }
              widget.ctrlWidget.setProp('model', model.ctrlWidget.widgetId);
              widget.ctrlWidget.attachedModel = model;
              model.ctrlWidget.modelTarget = widget;
              var props = widget.ctrlWidget.designPropertyValues();
              handleDefaultImage(props, widget.ctrlWidget.getWidgetTagName());
              widget.LoadFromURL(props.url || props.src);
              sessionStorage.setItem('elementTag', 'twx-dt-target-model-combined');
              sessionStorage.setItem('checkTarget' + currentView, true);
            } else {
              widget.HandleModelTargetError();
            }
          };
          ...
          widget.HandleModelTargetError = function () {
            const currentView = sessionStorage.getItem('currentView');
            sessionStorage.setItem('elementTag', '');
            if (sessionStorage.getItem('checkTarget' + currentView) !== 'true') {
              sessionStorage.setItem('checkTarget' + currentView, false);
            }
            ...
            setTimeout(() => {
              ctrl.deselectObject(widget.ctrlWidget);
              $rootScope.$broadcast('loadError3DObj', { ctrlWidget: widget.ctrlWidget });
              widget.ctrlWidget.remove();
              delete widget.ctrlWidget;
            }, 100);
          ...
          if (dropCoords) {
            ...
            handleDefaultImage(props, ctrlWidget.getWidgetTagName());
            let baseImage = props.modelTargetType === STANDARD ? SQUARE_BASEPLATE : CIRCLE_BASEPLATE;
            widget.LoadFromURL(baseImage);
            widget.size = { height: props.height, width: props.width, scale: props.scale };
          }
          widget.CreateVectorImages();
          widget.SetGizmos();
        ...
        ctrl.addImageMarker = function (ctrlWidget, dropCoords, disableMoveEvent = false) {
          ...
          if (
            tagName === 'twx-dt-target' ||
            tagName === 'twx-dt-target-image' ||
            tagName === 'twx-dt-target-user-defined'
          ) {
            ctrl.countTarget = ctrl.countTarget + 1;
          }
          ...
          if (
            ...
            ctrl.myView.DoPickWithCallback(dropCoords.x, dropCoords.y, true, false, function (pickResult) {
              ImageMarkerPlacementPickHandler(widget, pickResult, ctrl.myView, ctrl);
            });
          } else {
            var props = ctrlWidget.designPropertyValues();
            handleDefaultImage(props, tagName);
        ...
        /**
         * Adds a new waypoint to the canvas.
         * Checks if a wayfinder already exists in the scene and adds the waypoint as a child.
         * @param ctrlWidget Waypoint controller widget
         * @param dropCoords Canvas drop coordinates
         * @param disableMoveEvent Flag
         * @returns {*} Created waypoint widget
         */
        ctrl.addWaypoint = function (ctrlWidget, dropCoords, disableMoveEvent = false) {
          /**
           *  workaround for updating the location properties when the waypoint was moved by
           *  a bound model over its properties panel
           * @type {DebouncedFunc<(function(*): void)|*>}
           */
          let updateLocationProps = _.debounce(function (waypointCtrl, location) {
            ctrlWidget.setProps({
              x: location.position.x.toFixed(4),
              y: location.position.y.toFixed(4),
              z: location.position.z.toFixed(4),
            ...
            // for this cas it's mandatory to call scope apply otherwise the property values will not be updated
            $rootScope.$apply();
          }, 500);

          let onLocationChanged = function (waypointCtrl, location) {
            // only update the location properties if the waypoint itself is not the selected item
            // and the focus is on an input, otherwise this will create a loop
            if (
              selectedWidget &&
              selectedWidget.widgetId !== ctrlWidget.widgetId &&
              parent.document.activeElement.tagName === 'INPUT'
            ) {
              updateLocationProps(waypointCtrl, location);
            }
          };

          const widget = CreateImageMarker(ctrlWidget, dropCoords, ctrl.myScene, disableMoveEvent, onLocationChanged);

          function ctrlHelper(ctrl) {
            return {
              getDraggerMode: ctrl.getDraggerMode,
              DoPickWithCallback: function (x, y, invert, includeMarkups, callback) {
                ctrl.myView.DoPickWithCallback(x, y, invert, includeMarkups, callback);
              },
              getModelById: function (userId) {
                return ctrl.myWidgets[userId];
              },
            };
          }

          ctrlWidget.ctrlHelper = ctrlHelper(ctrl);
          if (dropCoords && Object.keys(ctrl.myWidgets).length > 1) {
            widget.dropCoords = dropCoords;
            ctrl.myScene.SetFloorPickable(true);
            ctrl.myView.DoPickWithCallback(dropCoords.x, dropCoords.y, true, false, function (pickResult) {
              let idPath = pickResult.GetIdPath();
              let cvWidget = pickResult.GetModel();

              let modelCtrlWidget = null;
              if (cvWidget) {
                modelCtrlWidget = ctrl.myWidgets[cvWidget.GetUserId()];
                ctrlWidget.setProp('model', modelCtrlWidget.widgetId);
                ctrlWidget.setProp('componentOccurrence', idPath);

                if (modelCtrlWidget) {
                  ctrlWidget.setProp('placeholder_img', ctrlWidget.$widgetDef.imgOnModelUrl);
                }
              ...
              ImageMarkerPlacementPickHandler(widget, pickResult, ctrl.myView, ctrl);
              ctrlWidget.delegate.waypointPlacementCompleted(ctrlWidget, modelCtrlWidget);
            });
          } else {
            let props = ctrlWidget.designPropertyValues();
            const tagName = ctrlWidget.getWidgetTagName();
            handleDefaultImage(props, tagName);
            widget.LoadFromURL(props.url || props.src);
            widget.size = { height: props.height, width: props.width, scale: props.scale, sx: props.sx, sy: props.sy };

            // re-binding model after loading
            let modelId = ctrlWidget.getProp('model');
            let modelCtrlWidget = ctrl.myWidgets[modelId];

            if (modelCtrlWidget) {
              ctrlWidget.delegate.waypointPlacementCompleted(widget.ctrlWidget, modelCtrlWidget);
            }
          }

          // adding label
          ctrlWidget.$waypointLabel = CreateImageMarker(ctrlWidget, dropCoords, ctrl.myScene, true);

          // make sure we re-set the $cvWidget as this is overwritten by the CreateImageMarker call for the label
          ctrlWidget.$cvWidget = widget;

          // create label properties
          ctrlWidget.labelProps = _.cloneDeep(ctrlWidget.designPropertyValues());
          ctrlWidget.labelProps.placeholder_img = undefined;
          ctrlWidget.labelProps.height = 0.01;
          ctrlWidget.labelProps.width = '';
          ctrlWidget.labelProps.text = ctrlWidget.getProp('widgetName');
          ctrlWidget.labelProps.class = 'waypoint-label';

          GenerateMarkupImage(ctrlWidget.$waypointLabel, ctrlWidget.labelProps);

          ctrlWidget.$waypointLabel.SetDrawType(Module.DrawType.WORLD_SPACE_BILLBOARD);
          ctrlWidget.$waypointLabel.SetPickable(false);
          ctrlWidget.$waypointLabel.SetVisibility(ctrlWidget.getProp('showLabel'));
          ctrlWidget.$waypointLabel.SetDecal(ctrlWidget.getProp('labelOnTop'));
          ...
          if (dropCoords) {
            ...
            if (!dropshadowEnabled) {
              ...
              viewController.showSnackbarMessage(
                window.i18next.t('@ptc/ves-ar-extension:spatial-target-auto-enabled-drop-shadow-msg')
              );
            }
          }
        ...
        ctrl.add3DButton = function (
          ...
          widget.GenerateButtonImage = function (props) {
            ...
            generateLabelAndIconImage(widget, props, widget.imageOverridesText, widget.smallIcon);
          };
        ...
        ctrl.addGroup = function (ctrlWidget, dropCoords, src = 'extensions/images/group.pvz') {
          ...
          widget.modelItems = [];
          widget.children = [];
          widget.childCommonProps = ['decal', 'occlude', 'opacity'];
          widget.inheritableProps = ['visible'];
          ...
          widget.HideChildProps = function (childCtrl) {
            if (childCtrl) {
              ...
              Object.keys(childProps).forEach(function (key) {
                ...
                if (widget.inheritableProps.some((p) => p === prop.name)) {
                  prop.inherit = true;
                  const value = prop.value;
                  if (parseBool(value)) {
                    childCtrl.setProp(key, inherit);
                  }
                }
              });
            }
          ...
          widget.ShowChildProps = function (childCtrl) {
            if (childCtrl) {
              ...
              Object.keys(childProps).forEach(function (key) {
                ...
                if (widget.inheritableProps.some((p) => p === prop.name)) {
                  prop.inherit = false;
                  if (isInherited(prop.value)) {
                    childCtrl.setProp(key, true);
                  }
                }
              });
          ...
          widget.SetTVObservers = function () {
            let ModelObserverClass = Module.ModelObserver.extend('ModelObserver', {
              ...
              OnLocationChange: function (loc) {
                if (widget.ctrlWidget.getWidgetTagName() === 'twx-dt-3dpanel') {
                  widget.UpdateAssetsPosition(loc.position, loc.orientation);
                }
                $rootScope.$broadcast('move3DObj', { ctrlWidget: widget.ctrlWidget, location: loc });
              },
              // Selection Callback
        ...
        ctrl.add3DPanel = function (ctrlWidget, dropCoords) {
          ...
          widget.UpdateAssetsPosition = function (position, orientation) {
          ...
          widget.SetAssetsAppearance = function () {
            if (!widget.ButtonsLoaded()) {
              return;
            }
        ...
        ctrl.add3DVideo = function (ctrlWidget, dropCoords) {
          ...
          widget.SetDimensions = function (height, width) {
            ...
            const props = { height: mediaBtnsSize / 3, width: mediaBtnsSize / 3 };
        ...
        /**
         * Add a 3D Document to the Canvas
         *
         * @param {Object} ctrlWidget The controller of the twxWidget being added
         * @param {Object} dropCoords The point at which the widget was dropped on the screen (if any).
         *                            Undefined if the system is adding the widget (e.g. when loading the project)
         */
        ctrl.add3DDocument = function (ctrlWidget, dropCoords) {
          const mediaBtnsSize = ctrlWidget.getProp('btnSize');
          let widget = ctrl.myScene.MakeModel();
          let lastDocHeight = -1;
          let lastDocWidth = -1;
          let documentHandler;
          let sourceFileHasBeenUpdated = false;
          let fileLoadedSuccessfully = false;
          let mediaButtonSizeSet = false;
          ...
          widget.minHeight = widget3dUtils.getDocumentWidgetHeight(widget3dUtils.documentPageSizes[4].height); // A5 Landscape height
          widget.minWidth = widget3dUtils.getDocumentWidgetWidth(
            widget3dUtils.documentPageSizes[widget3dUtils.defaultDocumentPageSizeIndex].width,
            false,
          ...
          widget.ModelLoadedHandler = function (success, isStructure, errors) {
            if (!isStructure && success) {
              ...
              widget.ctrlWidget.buttons = {
                scrollToTop: ctrl.add3DPressButton(
                  widget.ctrlWidget,
                  widget.dropCoords,
                  Object.assign({ imageSrc: 'extensions/images/3D_Document_Scroll_To_Top.png' }, params)
                ),
                pageUp: ctrl.add3DPressButton(
                  widget.ctrlWidget,
                  widget.dropCoords,
                  Object.assign({ imageSrc: 'extensions/images/3D_Document_Page_Up.png' }, params)
                ),
                pageDown: ctrl.add3DPressButton(
                  widget.ctrlWidget,
                  widget.dropCoords,
                  Object.assign({ imageSrc: 'extensions/images/3D_Document_Page_Down.png' }, params)
                ),
                pin: ctrl.add3DToggleButton(
                  widget.ctrlWidget,
                  widget.dropCoords,
              ...
              widget.ctrlWidget.images = {
                pageNumber: ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords),
                pageImage: ctrl.addImageMarker(widget.ctrlWidget, widget.dropCoords),
              };
              ...
              widget.UpdateSource(widget.ctrlWidget.designPropertyValues().src);

              // Prevents a glitch where the page image is pickable
              // without modifying GenerateMarkupImage().
              GenerateMarkupImage(widget.ctrlWidget.images.pageImage, {
                src: '',
              });
              GenerateMarkupImage(ctrlWidget.images.pageNumber, {
                src: '',
              });

              // Buttons are not yet fully loaded at this point.
            }
          };

          widget.UpdateSource = function (newSrc) {
            if (!widget.ImagesLoaded()) {
              return;
            }
            // Any change to src necessitates an image update for the page.
            sourceFileHasBeenUpdated = true;
            fileLoadedSuccessfully = false;
            if (!newSrc || newSrc === '') {
              GenerateMarkupImage(ctrlWidget.images.pageNumber, {
                src: '',
              });
              GenerateMarkupImage(ctrlWidget.images.pageImage, {
                src: '',
              });
            } else {
              try {
                documentHandler = pdfjsLib.getDocument(newSrc);
              } catch (_) {
                console.error('Failed to create a file handler for "' + newSrc + '".');
                return;
              }
              let numberOfPages;
              documentHandler.promise
                .then(function (docProps) {
                  numberOfPages = docProps.numPages;
                  let canvasLabelString = '1/' + String(numberOfPages);
                  let props = {
                    fontColor: 'rgba(255, 255, 255, 1);',
                    width: widget3dUtils.ButtonMinSize,
                    height: widget3dUtils.ButtonMinSize / 2,
                    fontsize: 20,
                  };

                  let pageNumberTextImage = widget3dUtils.generateLabelAndIconTexture(
                    undefined,
                    props,
                    canvasLabelString,
                    false
                  ...
                  GenerateMarkupImage(ctrlWidget.images.pageNumber, {
                    src: pageNumberTextImage,
                  });
                  fileLoadedSuccessfully = true;
                })
                .then(() => {
                  widget.UpdatePageImageScale(
                    widget3dUtils.getDocImageWidth(
                      Number(widget.ctrlWidget.getProp('width')),
                      ctrlWidget.getProp('hideControls'),
                      ctrlWidget.getProp('btnSize')
                    ),
                    widget3dUtils.getDocImageHeight(Number(widget.ctrlWidget.getProp('height')))
                  );
                })
                .catch((_) => {
                  GenerateMarkupImage(ctrlWidget.images.pageNumber, {
          ...
          widget.UpdatePageImageScale = function (docWidth, docHeight) {
            let imagesHaveNotLoaded = !widget.ImagesLoaded();
            let scaleAlreadyUpdated =
              docHeight === lastDocHeight && docWidth === lastDocWidth && !sourceFileHasBeenUpdated;

            if (imagesHaveNotLoaded || !fileLoadedSuccessfully || scaleAlreadyUpdated || !documentHandler) {
              // Spamming this function will drop the performance of the studio significantly.
              return;
            }

            lastDocHeight = docHeight;
            lastDocWidth = docWidth;
            sourceFileHasBeenUpdated = false;
            documentHandler.promise
              .then(function (docProps) {
                docProps.getPage(1).then(function (page) {
                  let originalViewport = page.getViewport({ scale: 1 });
                  let pageWidth = originalViewport.width;
                  let pageHeight = originalViewport.height;
                  let newPageImageHeight = widget3dUtils.calculatePageImageDimensions(
                    pageWidth,
                    pageHeight,
                    docWidth,
                    docHeight
                  ).height;
                  let newScale = (newPageImageHeight * 1000) / pageHeight; // 1000 conversion ratio
                  let scaledViewport = page.getViewport({ scale: newScale });
                  let canvas = document.createElement('canvas');
                  let context = canvas.getContext('2d');
                  ...
                  // Render PDF page into canvas context
                  let renderContext = { canvasContext: context, viewport: scaledViewport };
              ...
              .catch((_) => {
                console.error('Failed to obtain the properties of the existing document.');
                return; // Added in case this function will be appended.
              });
          };

          widget.UpdateAssetsPosition = function (position, orientation) {
            if (!widget.ImagesLoaded() || !widget.ButtonsLoaded()) {
              return;
            }
            ...
            const mediaBtnsImgsLoc = widget3dUtils.getPanelDocumentControlButtonsImagesAbsolutePositionsAndSize(
              position.x,
              position.y,
              position.z,
              orientation.x,
              ...
              height,
              mediaBtnsSize,
              widget.ctrlWidget.properties.hideControls.value
            );

            widget3dUtils.setPositionsToAssets(ctrlWidget.buttons, mediaBtnsImgsLoc, orientation);

            ctrlWidget.buttons.pin.SetPosition(pinBtnLocSize.x, pinBtnLocSize.y, pinBtnLocSize.z);
            ctrlWidget.buttons.pin.SetOrientation(orientation.x, orientation.y, orientation.z);

            widget3dUtils.setPositionsToAssets(ctrlWidget.images, mediaBtnsImgsLoc, orientation);
          };

          widget.SetMediaButtonsSize = function () {
            if (!widget.ButtonsLoaded() || mediaButtonSizeSet) {
              return;
            }
            mediaButtonSizeSet = true;
            // Set button image width and height to be half of the button
            const props = { height: mediaBtnsSize / 3, width: mediaBtnsSize / 3 };
            for (const btnKey in ctrlWidget.buttons) {
              if (btnKey === 'pin') {
                // Contrary to the rest, this button always adjusts its size.
                continue;
              }
              let currentButton = ctrlWidget.buttons[btnKey];
              let currentButtonImage = currentButton.$imageWidget;
              currentButton.SetButtonDimensions(mediaBtnsSize, mediaBtnsSize);
              SetHeightWidth(currentButtonImage, props);
              let isFirstLoad = currentButtonImage.firstload;
              let isPickable = currentButtonImage.GetPickable();
              // Rerun if the button is pickable or it's its first load.
              mediaButtonSizeSet = mediaButtonSizeSet && !isPickable && !isFirstLoad;
              // There is no need to rerun SetMediaButtonsSize if all of the buttons meet
              // these conditions since the media buttons size is constant.
            }
          };

          widget.SetPinButtonDimensions = function (width) {
            if (!widget.ButtonsLoaded()) {
              return;
            }
            const pinBtnSize = widget3dUtils.getPanelPinButtonSize(width);
          ...
          widget.SetDimensions = function (height, width) {
            const docWidgetHeight = Number(height) || widget.minHeight;
            const docWidgetWidth = Number(width) || widget.minWidth;

            widget.SetMediaButtonsSize();
            ...
            widget.UpdatePageImageScale(
              ...
              widget3dUtils.getDocImageHeight(docWidgetHeight)
            );

            widget.SetScaleXYZ(docWidgetWidth, docWidgetHeight, 1);

            widget.SetPinButtonDimensions(docWidgetWidth);
          };
          ...
          widget.ImagesLoaded = function () {
            if (!widget.ctrlWidget.images) {
              return false;
            }

            for (const imgKey in ctrlWidget.images) {
              if (!ctrlWidget.images[imgKey]) {
                return false;
              }
            }
            return true;
          ...
          widget.SetAssetsAppearance = function () {
            if (!widget.ButtonsLoaded() || !widget.ImagesLoaded()) {
              return;
            }

            for (const imgKey in ctrlWidget.images) {
              ctrlWidget.images[imgKey].SetPickable(false);
            }
          ...
          widget.DeleteButtons = function () {
            ...
            for (const btnKey in ctrlWidget.buttons) {
              ...
              if (currentButton && currentButton.$imageWidget) {
                ctrl.myScene.DeleteImageMarker(currentButton.$imageWidget);
                if (currentButton.$imageWidget) {
                  currentButton.$imageWidget.delete();
                }
              }
              if (currentButton) {
                ctrl.myScene.RemoveModel(currentButton);
              }
            }
            for (const imgKey in ctrlWidget.images) {
              ctrl.myScene.DeleteImageMarker(ctrlWidget.images[imgKey]);
              ctrlWidget.images[imgKey].delete();
            }
          };
        ...
        ctrl.addCustomWidget = function (ctrlWidget, dropCoords) {
          ...
          // Wayfinder widgets need to know the view with the camera
          if (ctrlWidget.getWidgetTagName() === 'twx-dt-wayfinder') {
            ctrlWidget.cameraView = ctrl.myView;
          }
     ...
     */
    function generateLabelAndIconImage(cvWidget, props, ignoreText = false, smallIcon = true) {
      if (!props.src) {
        // In case of legacy 3dButton widget we don't want to generate a texture with formated text
        if (props.text !== undefined && cvWidget.ctrlWidget.$widgetDef.elementTag !== 'twx-dt-3dbutton') {
          // Render text only
          props.src = widget3dUtils.generateLabelAndIconTexture(undefined, props, props.text, smallIcon);
          props.text = '';
        }
      ...
      background.onload = () => {
        props.src = widget3dUtils.generateLabelAndIconTexture(background, props, props.text, smallIcon);
        props.text = '';
        GenerateMarkupImage(cvWidget.$imageWidget, props);
      };
    }
     ...
     * @param {boolean} disableMoveEvent - disables the on location change handler
     * @param {function} onLocationChangedCallback - a custom callback for the location changed handler
     */
    function CreateImageMarker(
      ...
      scene,
      disableMoveEvent = false,
      onLocationChangedCallback = undefined
    ) {
      ...
      let ImageMarkerObserverClass = Module.ImageMarkerObserver.extend('ImageMarkerObserver', {
        OnLocationChange: function () {
          if (!disableMoveEvent) {
            ...
            if (widget.ctrlWidget.$cvWidget) {
              const loc = widget.ctrlWidget.$cvWidget.GetLocation();
              loc.scale.z = loc.scale.x; // z scale isn't relevant in ImageMarker (DT-21705)

              $rootScope.$broadcast('move3DObj', {
                ctrlWidget: widget.ctrlWidget,
                location: loc,
              });

              if (onLocationChangedCallback) {
                onLocationChangedCallback(widget.ctrlWidget, loc);
              }
            }
          }
        },
      ...
      widget.ImageMarkerLoaded = function (bool) {
        let firstload = widget.firstload;
        ...
        $rootScope.$broadcast('imageMarkerLoaded', {
          widget: widget,
          ctrlWidget: widget.ctrlWidget,
          firstload: firstload,
        });
     ...
     */
    function ImageMarkerPlacementPickHandler(widget, result, view, ctrl) {
      widget.scene.SetFloorPickable(false);
      var position = { x: 0, y: 0, z: 0 };
      var orientation = { x: 0, y: 0, z: 0 };
      var model = result.GetModel();

      const panelParent = getPanelWidgetOfModel(ctrl, model);
      if (panelParent && widget.ctrlWidget && !widget.ctrlWidget.getWidgetTagName().includes('target')) {
        panelParent.element()[0].appendChild(widget.ctrlWidget.element()[0]);
        panelParent.$cvWidget.SetChildren();
      }
      ...
      if (result.IsValid()) {
        ...
        if (panelParent && widget.ctrlWidget && !widget.ctrlWidget.getWidgetTagName().includes('target')) {
          loc = getRelativeLocation(panelParent, loc, panelParent.$cvWidget.getDimensions());
          loc.position.z = 0;
          loc.orientation = { x: 0, y: 0, z: 1 };
        }
      ...
      // Load the image
      var props = widget.ctrlWidget.designPropertyValues();
      const targets = ['twx-dt-image', 'twx-dt-target', 'twx-dt-target-image', 'twx-dt-target-spatial'];
      if ((targets.includes(tagName) || isModelTargetTag(tagName)) && tagName !== 'twx-dt-target-model-combined') {
        handleDefaultImage(props, tagName);
        widget.LoadFromURL(props.url || props.src);
      } else {
        GenerateMarkupImage(widget, props);
    ...
    function UpdateModelTargetLocation(widget, recalculateOffset) {
      ...
      if (recalculate) {
        ...
        if (box.valid) {
          ...
          if (typeof widget.AdjustVectorImagesToModel === 'function') {
            widget.AdjustVectorImagesToModel(box, model_loc);
          }
        }
      }
  ...
  function Update3DButtonLabelLocation(modelWidget, imageWidget) {
    ...
    if (box.valid) {
      ...
      if (location) {
        let buttonTransform = widget3dUtils.transformFromPose(location);
        let imageTransform = widget3dUtils.transformFromPose({ translation: { x: 0.0, y: 0.0, z: 0.0001 } });
        let globalImageTransform = buttonTransform.multiply(imageTransform);
        let imagePose = widget3dUtils.poseFromTransform(globalImageTransform);
        imageWidget.SetPosition(imagePose.translation.x, imagePose.translation.y, imagePose.translation.z);
        imageWidget.SetOrientation(imagePose.rotation.x, imagePose.rotation.y, imagePose.rotation.z);
      }
    }
  }
  ...
  function getPanelWidgetOfModel(ctrl, model) {
    if (model === null || model === undefined) {
      return undefined;
    }
    const widgetIds = Object.keys(ctrl.myWidgets);
    for (let i = 0; i < widgetIds.length; i++) {
      const ctrlWidget = ctrl.myWidgets[widgetIds[i]];
      const cvWidget = ctrlWidget.$cvWidget;
      if (ctrlWidget.getWidgetTagName() === 'twx-dt-3dpanel' && cvWidget && cvWidget.$$.ptr === model.$$.ptr) {
        return ctrlWidget;
      }
    }
  }
   ...
   */
  function ModelPlacementPickHandler(widget, pickResult, ctrl) {
    widget.scene.SetFloorPickable(false);
    let position = { x: 0, y: 0, z: 0 };
    var model = pickResult.GetModel();
    ...
    if (pickResult.IsValid()) {
      ...
      let loc = pickResult.GetLocation();
      if (panelParent) {
        loc = getRelativeLocation(panelParent, loc, panelParent.$cvWidget.getDimensions());
        loc.position.z = 0;
      }
  ...
  function getRelativeLocation(parent, absLocation, dimensions) {
    if (parent.$cvWidget && absLocation) {
      // return the absolute location in case the parent has no location
      if (!parent.$cvWidget.GetLocation) {
        return absLocation;
      }
  ...
  function handleDefaultImage(props, widgetTag) {
    if (props.placeholder_img) {
      ...
      const name =
        widgetTag === 'twx-dt-waypoint' || widgetTag === 'twx-dt-image' || widgetTag === 'twx-dt-sensor'
          ? 'src'
          : 'url';
  ...
  function GenerateMarkupImage(widget, props) {
    ...
    if (!props.src || props.src.slice(-1) === '/') {
      ...
      image.onload = function () {
        // only render the image if the widget was not deleted while the image loading occurred
        if (widget.ctrlWidget && widget.ctrlWidget.$cvWidget) {
          RenderImageAndText(widget, props, image);
        } else {
          console.warn('Did not render image as the widget was deleted during loading.', widget.ctrlWidget.widgetId);
        }
      };
        ...
        Children: [
          ...
          },
          {
            ...
            Children: [
              ...
              {
                Type: 'preference_list',
                Name: 'Selection highlight styles',
                Locked: false,
                Children: [
                  {
                    Type: 'preference_item',
    ...
    if (views && views.length) {
      ...
      var timerId = setInterval(function () {
        tv_view.GetRenderStats(function (rs) {
          const text = window.parent.i18next.t('@ptc/ves-ar-extension:scene-polygon-count', {
            number: rs.triangleCount,
          });
  ...
  function setModelTargetWidth(modelTarget, boundingBox) {
    ...
    modelTarget.SetWidth(1.1 * Math.max(boundingBox.max.x - boundingBox.min.x, boundingBox.max.z - boundingBox.min.z));
  }
