...
(function() {
        ...
        var loadTrackerDefSuccessCallback = function (dataset, element) {
            try {
                ...
                if (enableTrackingEvents) {
                    renderer.setupTrackingEventsCommand(function (trackerId, position, gaze, up, timestamp) {
                ...
                scope.target.addAsset = function(tracker) {
                    ...
                    VF_ANG.addMarker(renderer, element, function() {
                        ...
                        renderer.addMarker(tracker.name,
                                           ...
                                           successCallback,
                                           failureCallback);
                        if (typeof(renderer.addTargetRepresentation) === "function") {
                            if (element.attr("showRepresentation") === "true") {
                                renderer.addTargetRepresentation({
                                    target: element.attr("id"),
                                    src: element.attr("representation-src"),
                                    tracker: tracker.name
                                });
                            }
                        }
    ...
    vuforiaModule.directive('twxDtLabel', [ 'tml3dRenderer', '$animate', function (renderer, $animate) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                var attributeNames = [ 'class', 'text', 'textattrs', 'fontFamily', 'fontColor', 'fontOutlineColor' ];
    ...
    vuforiaModule.directive('twxDtVideo', [ 'tml3dRenderer', '$animate', function (renderer, $animate) {
        return {
            restrict: 'E',
            require: '^twxDtTracker',
            link: function (scope, element, attrs, ctrl) {
                ...
                var addFunction = function addFunction (addFunctionAddAPICallCallback, addFunctionSuccessCallback,
                                                        addFunctionFailureCallback) {
                    var asset = VF_ANG.createObj("twx-dt-video", element);
                    $animate.enabled(element, false);
                    ...
                    asset.addAsset = function() {
                        ...
                        var addFailureCallback = function (error) {
                            ...
                            console.log("Error in add3DVideo for [" + element.attr("id") + "] due to [" + error + "]");
                        };

                        if (typeof(renderer.add3DVideo) === "function") {
                            VF_ANG.addAsset(renderer, asset, function() {
                                addFunctionAddAPICallCallback();
                                ...
                                renderer.add3DVideo(params,
                                                    addSuccessCallback,
                                                    addFailureCallback);
                            });
                        } else {
                            failureCallback("add3DVideo not supported by the current platform");
                        }
                    };
                ...
                var setFunction = function setFunction () {
                    var params = {
                            "id" : element.attr('id'),
                            "src" : encodeURI(element.attr('src')),
                            "preload" : element.attr('preload')
                        };
                    renderer.setVideo(params);
                };

                var attributeNames = [ "src", "preload" ];
                VF_ANG.setupWatchWithAddSetConvention(scope, element, attributeNames, addFunction, setFunction);
            }
        };
    }]);

    vuforiaModule.directive('twxDtDocument', [ 'tml3dRenderer', '$animate', function (renderer, $animate) {
        return {
            restrict: 'E',
            require: '^twxDtTracker',
            link: function (scope, element, attrs, ctrl) {
                ...
                var addFunction = function addFunction (addFunctionAddAPICallCallback, addFunctionSuccessCallback,
                                                        addFunctionFailureCallback) {
                    var asset = VF_ANG.createObj("twx-dt-document", element);
                    $animate.enabled(element, false);
                    ...
                    asset.addAsset = function() {
                        ...
                        var addFailureCallback = function (error) {
                            ...
                            console.log("Error in add3DDocument for [" + element.attr("id") + "] due to [" + error + "]");
                        };

                        if (typeof(renderer.add3DDocument) === "function") {
                            VF_ANG.addAsset(renderer, asset, function() {
                                addFunctionAddAPICallCallback();
                                ...
                                renderer.add3DDocument(params,
                                                    addSuccessCallback,
                                                    addFailureCallback);
                            });
                        } else {
                            failureCallback("add3DDocument not supported by the current platform");
                        }
                    };
    ...
    vuforiaModule.directive('twxDtAudioplayback', [ 'tml3dRenderer', '$animate', function (renderer, $animate) {
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                var attributeNames = ['src', 'preload', 'spatial'];
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                var setFunction = function setFunction (newValues, oldValues) {
                    ...
                    var loadAndRedraw = function () {
                        ...
                        var newText = element.attr('text');

                        if (currentText !== null && currentText === newText && currentImage !== null && newValue === currentImage.origsrc &&
...
VF_ANG.applyWatch = function(scope,element,renderer,id) {
    ...
    scope.$watch(function () { return element.attr('width') + element.attr('height'); }, function (value) {
        if (typeof(renderer.setDimensions) === "function") {
            renderer.setDimensions(nodeId, {
                width: element.attr('width'),
                height: element.attr('height')
            });
        }
    ...
    scope.$watch(function () { return nodeId +
                                      element.attr('billboard') + element.attr('tagalong') + element.attr('tagalong-snapping-distance') + element.attr('tagalong-offset') +
                                      element.attr('occlude') + element.attr('hidden') + element.attr('phantom') + element.attr('opacity') + element.attr('decal') + element.attr('shader') +
                                      element.attr('force-hidden') + element.attr('experimental-one-sided'); }, function (value) {
        ...
        renderer.setProperties(nodeId, {
            ...
            tagalongSnappingDistance: element.attr('tagalong-snapping-distance'),
            tagalongOffset: element.attr('tagalong-offset'),
...
VF_ANG.getiOSInfo = function getiOSInfo() {
    var iosInfo ={};
    // eg. user agent for iPhone device 'VuforiaView/1.0.0 (iPhone9,3; iOS15.0.1)'
    if (navigator.userAgent.match(/ipad|iphone|ipod/i)) {
        iosInfo.userAgent=navigator.userAgent;
        // full release - iOS15.0.1
        iosInfo.fullRelease=(navigator.userAgent).match(/iOS(\d)?\d.\d(.\d)?/i)[0];
        // major release - 15
        iosInfo.majorReleaseString=(navigator.userAgent).match(/iOS(\d)?\d.\d(.\d)?/i)[0].split('.')[0].replace("iOS","");
    }
    return iosInfo;
};
...
VF_ANG.getTextAttrs = function getTextAttrs(textPropertyString, fontFamily, fontColor, fontOutlineColor, defaultFontSize, element) {
    ...
    if (element) {
        ...
        if (computed) {
            ...
            var iOSInfo = VF_ANG.getiOSInfo();
            if (iOSInfo.majorReleaseString === '15') {
                // TODO: Remove this once iOS 15 issue with Japanese text is fixed from apple side.
                // Check DT-35109 ticket
