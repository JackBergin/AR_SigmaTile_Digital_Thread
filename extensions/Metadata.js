 ...
 */
  ...
  PTC.Metadata = class Metadata {
    ...
    static fromId(id, cb) {
      ...
      return md._updateDataIfNecessary(cb);
    }
    ...
    _updateDataIfNecessary(cb) {
      var self = this;
      return new Promise(function (resolve, reject) {
        if (!_propertiesCache[self.id] || self.isExternalData) {
          const modelEl = document.querySelector('[id="' + self.id + '"]');
          if (modelEl) {
            const src = modelEl.getAttribute('src');
            const properties = _propertiesCache[src];
            if (!properties) {
              if (self.isExternalData && (!_propertiesCache[self.id] || _propertiesCache[self.id].isExternalData)) {
                ...
                http
                  ...
                  .error(function (error) {
                    ...
                    // self._metadataListener(resolve, cb);
    ...
    _metadataListener(resolve, cb) {
      let self = this;
      const cbFunction = (event) => {
        if (cb) {
          cb(event.detail);
        } else {
          document.removeEventListener(`${self.id}_metadataChanged`, cbFunction);
      ...
      if (self.observer) {
        ...
        document.addEventListener(`${self.id}_metadataChanged`, cbFunction);
      }
    }
