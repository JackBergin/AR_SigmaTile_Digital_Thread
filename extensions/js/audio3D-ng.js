      ...
      link: function (scope, element, attr) {
        ...
        let relocateWidgetElements = function (width, height, showControlsFlag) {
          ...
          if (twx.app.isPreview()) {
            scope.renderer.setDimensions(panelId(), {
              width: width,
              height: height,
            });
            ...
            if (showControlsFlag) {
              scope.renderer.setDimensions(prevBtnId(), {
                width: mediaBtnsPosSize.size,
                height: mediaBtnsPosSize.size,
              });
              ...
              scope.renderer.setDimensions(nextBtnId(), {
                width: mediaBtnsPosSize.size,
                height: mediaBtnsPosSize.size,
              });
              ...
              scope.renderer.setDimensions(stopBtnId(), {
                width: mediaBtnsPosSize.size,
                height: mediaBtnsPosSize.size,
              });
              ...
              scope.renderer.setDimensions(playBtnId(), {
                width: mediaBtnsPosSize.size,
                height: mediaBtnsPosSize.size,
              });
        ...
        function isEmptyOrSupportedAudioSource() {
          return (
            (scope.srcField && scope.data.src !== scope.srcField && widget3dUtils.isSupportedAudio(scope.srcField)) ||
            scope.srcField === ''
          );
        }

        scope.$watch('srcField', function () {
          if (isEmptyOrSupportedAudioSource()) {
