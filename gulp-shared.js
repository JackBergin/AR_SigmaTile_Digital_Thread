...
const log = require('@ptc/studio-log').getLogger('ar-extension:gulp-shared');
const zipHelper = require('./zipHelper.js');
const ves_extension = require('@ptc/ves-ar-extension');
...
function updateMetadataWithWidgetsUsage(metadata, combinedWidgetsUsage, saveConfig) {
  ...
  const lastSave = metadata.widgetsUsage[metadata.widgetsUsage.length - 1];
  if (lastSave && lastSave.action === 'LATEST_SAVE') {
    lastSave.timestamp = currentTime;
    lastSave.usage = combinedWidgetsUsage;
  } else {
    metadata.widgetsUsage.push({ timestamp: currentTime, action: 'LATEST_SAVE', usage: combinedWidgetsUsage });
  }
  ...
  if (output) {
    ...
    if (params.settings.projectType === 'eyewear') {
      var appEventsOverlay =
        ...
        '      document.querySelector(".empty-app-events").innerText = parent.i18next.t("@ptc/ves-ar-extension:No Application Events to execute"); ' +
        '    </script>' +
        '  </div>' +
        '</div>';
 ...
 */
function handleResourceUrl(propValue, saveConfig, property, shouldEncode) {
  if (!propValue.startsWith('http') && !propValue.startsWith('data:image')) {
    if (saveConfig.projectSettings.designedForOffline && (property.inlineForOffline || /\.svg$/i.test(propValue))) {
      try {
        ...
        propValue = 'app/resources/' + (shouldEncode ? encodeURI(propValue) : propValue);
      }
    } else {
      propValue = 'app/resources/' + (shouldEncode ? encodeURI(propValue) : propValue);
    }
 ...
 * @private
 */
function compileViewContents(contents, viewName, viewTitle, saveConfig, params, sharedContent, convertedModels) {
        ...
        _.each(properties, function (property) {
          ...
          if (datatype === 'custom_ui') {
            ...
            if (datatype !== undefined) {
              ...
              switch (datatype.toLowerCase()) {
                ...
                case 'boolean':
                  if (propValue === '') {
                    propValue = property.default;
                  } else if (Number(propValue) === inherit) {
                    propValue = inherit;
            ...
            if (datatype === 'resource_url' && propValue) {
              propValue = handleResourceUrl(propValue, saveConfig, property, true);
            }
...
function compileViewContents_step2(compiledDoc, params, config) {
  ...
  if (isEyewear || isDesktop) {
    ...
    // this flag informs Ionic that we want to use native-scroll instead of ionic-scroll
    // we need this because there is an issue with a nested list on iOS with ionic-scroll
    overflowScroll = 'overflow-scroll="true" ';
  }
...
exports.saveNewMetaDataContent = saveNewMetaDataContent;
